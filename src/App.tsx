...
import db from './db.json'; // Assuming you have a db.json file with initial data
...
const App: React.FC = () => {
  const [wholesaleData, setWholesaleData] = useState<WholesaleProduct[]>(db as WholesaleProduct[]);
  // Extract unique categories from db data
  const [categories, setCategories] = useState<string[]>(() => {
    const uniqueCategories = Array.from(new Set(db.map((item: WholesaleProduct) => item.category)));
    return uniqueCategories.length > 0 ? uniqueCategories : ['Default'];
  });
